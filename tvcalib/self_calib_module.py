...
from tvcalib.cam_modules import (
    CameraParameterWLensDistDictLinear,
    CameraParameterWLensDistDictZScore,
    SNProjectiveCamera,
)
from tvcalib.utils.linalg import (
    distance_finite_line_pointcloud,
    distance_line_pointcloud_3d,
    distance_point_pointcloud,
)


class TVCalibOptim(torch.nn.Module):
    def __init__(
        self,
        model3d,
        cam_distr,
        ...
        **kwargs,
        ...
        # self.optim = torch.optim.Adam(self.cam_param_dict.param_dict.parameters(), lr=0.1)
        ...
        if self.lens_distortion_active:
            ...
            #     lr_lambda=lambda step: step / self.optim_steps,
            # )
            self.Scheduler_lens_distortion = partial(
                torch.optim.lr_scheduler.OneCycleLR,
                max_lr=1e-3,
    ...
    def forward(self, x):
        ...
        # distances_px_lines_raw = distance_finite_line_pointcloud(lp1, lp2, pc)
    ...
    def self_optim_batch(self, x, *args, **kwargs):
        ...
        with tqdm(range(self.optim_steps), **self.tqdm_kwqargs) as pbar:
            for step in pbar:
                ...
                # stopping criteria for each sample
                ...
                pbar.set_postfix(
                    loss=loss_total_dist.detach().cpu().tolist(),
                    loss_lines=losses["loss_ndc_lines"].detach().cpu().tolist(),
                    loss_circles=losses["loss_ndc_circles"].detach().cpu().tolist(),
                )
